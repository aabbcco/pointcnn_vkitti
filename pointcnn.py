...
def xconv(pts, fts, qrs, tag, N, K, D, P, C, C_pts_fts, is_training, with_X_transformation, depth_multiplier,
    ...
    if with_X_transformation:
        ######################## X-transformation #########################
        X_0 = pf.conv2d(nn_pts_local, K * K, tag + 'X_0', is_training, (1, K))  #（N，P，1，K*K）   conv2d with relu
        X_0_KK = tf.reshape(X_0, (N, P, K, K), name=tag + 'X_0_KK')             #（N，P，K，K）
        X_1 = pf.depthwise_conv2d(X_0_KK, K, tag + 'X_1', is_training, (1, K))  #（N，P，1，K*K）？  like small inception?
        X_1_KK = tf.reshape(X_1, (N, P, K, K), name=tag + 'X_1_KK')
        X_2 = pf.depthwise_conv2d(X_1_KK, K, tag + 'X_2', is_training, (1, K), activation=None)#(N,P,1,K*K)?
        X_2_KK = tf.reshape(X_2, (N, P, K, K), name=tag + 'X_2_KK')             # X matrix
        fts_X = tf.matmul(X_2_KK, nn_fts_input, name=tag + 'fts_X')             # X conv?
        ###################################################################
    else:
        fts_X = nn_fts_input
