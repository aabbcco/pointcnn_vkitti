...
def main():
    ...
    parser.add_argument('--h5_num', '-d', help='Number of h5 files to be loaded each time', type=int, default=16)
    ...
    for split in splits:
        split_filelists[split] = ['./%s/%s\n' % (split, filename) for filename in os.listdir(os.path.join(root, split)) if filename.endswith('.h5')]
    ...
    with open(train_list, 'w') as filelist:
        list_num =int(math.ceil(len(train_h5) / args.h5_num))
        for list_idx in range(int(list_num)):
