...
def main():
    ...
    parser.add_argument('--max_point_num', '-m',
                        help='Max point number of each sample', type=int, default=512)
    parser.add_argument('--block_size', '-b',
                        help='Block size', type=float, default=3.0)
    parser.add_argument('--grid_size', '-g',
                        help='Grid size', type=float, default=0.1)
    parser.add_argument('--save_ply', '-s',
                        help='Convert .pts to .ply', action='store_true')

    args = parser.parse_args()
    print(args)

    root = args.folder if args.folder else '../../data/vkitti3d'
    ...
    indices_split_to_full = np.zeros(
        (batch_size, max_point_num), dtype=np.int32)
    ...
    for folder in folders:
        datasets = [filename[:-4]
                    for filename in os.listdir(folder) if filename.endswith('.npy')]
        for dataset_idx, dataset in enumerate(datasets):
            filename_txt = os.path.join(folder, dataset + '.npy')
            print('{}-Loading {}...'.format(datetime.now(), filename_txt))
            xyzrgbl = np.load(filename_txt)
            xyz, rgb, labels = np.split(xyzrgbl, (3, 6), axis=-1)
            ...
            for offset_name, offset in offsets:
                ...
                block_point_indices = np.split(np.argsort(
                    point_block_indices), np.cumsum(block_point_counts[:-1]))
                print('{}-{} is split into {} blocks.'.format(datetime.now(),
                                                              dataset, blocks.shape[0]))
                ...
                for block_idx in range(blocks.shape[0]):
                    ...
                    for x, y in nbr_block_offsets:
                        ...
                        break
                print('{}-{} of {} blocks are merged.'.format(datetime.now(),
                                                              block_merge_count, blocks.shape[0]))
                ...
                for block_idx in range(idx_last_non_empty_block + 1):
                    ...
                    grids, point_grid_indices, grid_point_counts = np.unique(xyz_grids, return_inverse=True,
                                                                             return_counts=True, axis=0)
                    grid_point_indices = np.split(np.argsort(
                        point_grid_indices), np.cumsum(grid_point_counts[:-1]))
                ...
                for block_idx in range(idx_last_non_empty_block + 1):
                    ...
                    block_xzyrgb = np.concatenate(
                        [x, z, y, rgb[point_indices]], axis=-1)
                    ...
                    for block_split_idx in range(block_split_num):
                        ...
                        blables=block_labels[start:end]
                        label_seg[idx_in_batch,0:point_num] = blables.reshape(-1)
                        indices_split_to_full[idx_in_batch,0:point_num] = point_indices[start:end]
                        if args.save_ply:
                            block_center_xzy = np.array(
                                [[block_center[0][0], block_center[0][2], block_center[0][1]]])
                            data_center[idx_in_batch,
                                        0:point_num, ...] = block_center_xzy
                        ...
                        if ((idx + 1) % batch_size == 0) or \
                            ...
                            filename_h5 = os.path.join(
                                folder, dataset + '_%s_%d.h5' % (offset_name, idx_h5))
                            print(
                                '{}-Saving {}...'.format(datetime.now(), filename_h5))

                            file = h5py.File(filename_h5, 'w')
                            file.create_dataset(
                                'data', data=data[0:item_num, ...])
                            file.create_dataset(
                                'data_num', data=data_num[0:item_num, ...])
                            file.create_dataset(
                                'label', data=label[0:item_num, ...])
                            file.create_dataset(
                                'label_seg', data=label_seg[0:item_num, ...])
                            file.create_dataset(
                                'indices_split_to_full', data=indices_split_to_full[0:item_num, ...])
                            file.close()

                            if args.save_ply:
                                print(
                                    '{}-Saving ply of {}...'.format(datetime.now(), filename_h5))
